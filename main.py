...
async def load_and_send_board(message, game, content=None, show_range=False):
    game.display(f"./maps/{message.channel.id}.png", who_id=message.author.id if show_range else None, box_size=board_size, thickness=2)
...
''')
            ...
            if args[0].casefold() == ".join":
                ...
                if game.active():
                    await load_and_send_board(message, game, None, "-r" in message.content)
                else:
